...
3. Скрипт ставит в `~/Library/Services` два workflow:
   ...
   - **Правило `pick_ext_auto`:** **`.oz`** — папки и «текстовые» расширения (в т.ч. **`epub`**, `txt`, `md`, исходники, `json`, `xml` …); **`.zip`** — остальное (в т.ч. `pdf`, изображения, видео). Список задаётся в `scripts/install-context-menu.sh`.
